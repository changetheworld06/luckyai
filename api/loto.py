...
def generate_loto_grids() -> GenerateResponse:
    """
    Génère 10 grilles Loto optimisées (version 1 : algo simple + score).
    """
    # Paramètres Loto (France)
    all_main = list(range(1, 50))      # 1–49
    all_specials = list(range(1, 11))  # 1–10
    main_count = 5
    special_count = 1

    csv_path = os.path.join("data", "loto_history.csv")
    ...
    try:
        ...
        print("[LuckyAI] Tirages Loto chargés :", len(draws))
    except FileNotFoundError:
        # Fallback : pas d'historique -> stats neutres
